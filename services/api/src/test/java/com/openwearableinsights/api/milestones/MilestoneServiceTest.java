...
/**
 * Unit tests for {@link MilestoneService}: every one of the four checks
 * (habit streak, best tracked week of steps, VO2max high, strength-training
 * period) fires only on a genuine "current beats/ties every real prior
 * value" comparison, and the honest empty-list result when nothing
 * qualifies. All four collaborating services are mocked directly — their
 * own methodologies are already covered by their own unit tests; this only
 * verifies how {@link MilestoneService} composes their real outputs.
 *
