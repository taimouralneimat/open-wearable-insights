...
graph TB
  subgraph Browser[User device — loopback]
    Flutter[Flutter web/iOS/Android<br/>go_router · Dio · fl_chart]
  end

  subgraph Loopback[127.0.0.1 — local only]
    API[Spring Boot API<br/>Java 21 · Spring Modulith<br/>Spring Security · OpenAPI 3 · Flyway · Micrometer]
  ...
  API -->|RestClient · constrained prompt| Ollama
...
|---|---|---|---|
| Flutter client | Flutter, go_router, Dio, fl_chart | browser/app | Web first; iOS/Android later |
| Spring Boot API | Java 21, Spring Boot 3.5.16, Spring Modulith, Spring Security, OpenAPI 3, Flyway, Micrometer | `127.0.0.1:8080` | Modular monolith; 15 target modules; talks to Ollama directly via Spring's `RestClient`, not Spring AI (see parity-matrix.md row 7) |
...
4. Insights module produces deterministic insight; coach module optionally
   requests LLM rephrasing via a direct `RestClient` call → Ollama (loopback).
